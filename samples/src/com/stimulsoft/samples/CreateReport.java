...
public class CreateReport extends JPanel {
    ...
    public CreateReport(final JFrame parentFrame) throws FileNotFoundException {
        ...
        StiXmlDatabase xmlDatabase = new StiXmlDatabase("Demo", StiResourceUtil.getStream(xsdPath), StiResourceUtil.getStream("/samples/Demo.xml"));
        report.setDictionary(new StiDictionary(report));
        report.getDictionary().getDatabases().add(xmlDatabase);

        StiXmlTableFildsRequest tables = StiDataColumnsUtil.parceXSDSchema(StiResourceUtil.getStream(xsdPath));
